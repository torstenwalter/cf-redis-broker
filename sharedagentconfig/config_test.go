package sharedagentconfig_test
	...
	Describe("Load", func() {
		Context("When the file does not exist", func() {
			It("returns an error", func() {
				_, err := sharedagentconfig.Load("/this/is/an/invalid/path")
		...
		Context("When the file is successfully loaded", func() {
			var config *sharedagentconfig.Config

			BeforeEach(func() {
				path, err := filepath.Abs(path.Join("assets", "agent.yml"))
				Expect(err).ToNot(HaveOccurred())

				config, err = sharedagentconfig.Load(path)
				Expect(err).ToNot(HaveOccurred())
			})
			...
			It("Has the correct conf_path", func() {
				Expect(config.ConfBasePath).To(Equal("/conf/path"))
			})
